...
func (s *Server) initI18n(engine *gin.Engine) error {
	...
	findI18nParamNames := func(key string) []string {
		...
		for i := 0; i < keyLen-1; i++ {
			if key[i:i+2] == "{{" { // проверка начала "{{"
				j := i + 2
				isFind := false
				for ; j < keyLen-1; j++ {
					if key[j:j+2] == "}}" { // проверка конца "}}"
						isFind = true
						break
					}
				}
...
func (s *Server) startTask() {
	...
	// проверка каждые 30 секунд, запущен ли xray
	s.cron.AddJob("@every 30s", job.NewCheckXrayRunningJob())

	go func() {
		time.Sleep(time.Second * 5)
		// статистика трафика каждые 10 секунд, первая задержка 5 секунд, не совпадает со временем перезапуска xray
		s.cron.AddJob("@every 10s", job.NewXrayTrafficJob())
	}()

	// проверка каждые 30 секунд превышения трафика и истечения срока действия inbound
	s.cron.AddJob("@every 30s", job.NewCheckInboundJob())
	// уведомление о трафике раз в день, время Шанхая 8:30
...
func (s *Server) Start() (err error) {
	// Это анонимная функция без имени
	defer func() {
		if err != nil {
			s.Stop()
		}
